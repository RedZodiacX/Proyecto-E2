...
const Actualizar = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const res = await fetch(`${process.env.REACT_APP_API}/api/products/${IdDocumento}&${IdSede}`, {
            method: "PUT",
            headers: {
...
const [Data, setData] = useState([]);
    ...
    const handleSubmit2 = async (e) => {
        e.preventDefault();
        const res = await fetch(`${process.env.REACT_APP_API}/api/products/${IdDocumento3}&${IdSede3}`, {
            method: "GET"
        });
        const data = await res.json();
            setData(data);
